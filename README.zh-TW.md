...
- **來源權威加權** —— 在 front matter 標註文件的可信度（`source_type: policy | faq | transcript | …`）；相關度接近時，權威來源會被重新排到前面，但不會蓋過明顯更相關的命中（[細節](#檢索流程)）。
...
3. **權威重排**。將每個候選的分數乘上 `(1 + priority × 0.05)`，其中 `priority` 來自檔案的 `source_type` front matter（`policy`/`official` = 3、`faq`/`guide` = 2、未標註 = 1、`transcript`/`chat`/`qa` = 0）。乘法因子對 BM25、餘弦、RRF 三種尺度都通用，之後再取前 k 段。這讓官方文件能在相關度接近時贏過逐字稿，但不會蓋過明顯更相關的段落。（LLM Index 的挑選沒有可比分數，故略過此步。）
4. **組裝提示**。將命中段落與其標題路徑（如 `refund_policy.md > 退款政策 > 退款時程`）合併至系統 + 使用者提示模板。
5. **生成回答**。使用 `streamText` 並將結果以 UI message 串流回傳。
...
但門檻只能控制「檢索到什麼」，擋不住模型在檢索內容之外自行加油添醋。所以還有第二層：答案生成後，**grounding 驗證器**（另一次 structured LLM 呼叫）把答案拆成原子主張，逐一比對檢索到的上下文，回傳 `{ grounded, unsupported[] }`。`/chat/stream` 會把結果以尾端的 `data-grounding` 區塊串流出來（答案即時串流，驗證徽章隨後出現）；`/chat` 則直接回傳。它採 fail-open——驗證器出錯不會誤殺正確答案——所以是把「沒依據的主張」標示出來，而非直接擋掉。

頭尾還各有一道防線。進來的查詢會先被掃描 **prompt-injection / 角色劫持** 樣式（如「ignore previous instructions」「reveal your system prompt」），命中就在檢索與 LLM 呼叫之前直接拒答；系統提示也把 context 與問題標記為不可信的資料、而非指令。出去時，答案的 inline 引用會跟檢索結果核對——亂編的 id 剝除，若一個都不剩就補回最相關的來源（套用於 `/chat`；`/chat/stream` 因文字已即時串流，改靠系統提示 + grounding 驗證器）。injection 樣式刻意避開單純關鍵字（「password」「api key」），確保正常的客服問題不會被誤擋。
